...
export function Page({ children }: { children: React.ReactNode }) {
  return <div className={styles.portfolio}>
    ...
    <header className={styles.header}><a className={styles.wordmark} href="/" aria-label="Hunain Parekh home"><span className={styles.monogram}>hp.</span><span>Hunain Parekh<small>AI process automation engineer</small></span></a><nav aria-label="Main navigation"><a href="/work">Case studies</a><a href="/approach">How I work</a><a href="/about">Journey</a></nav><a className={styles.headerContact} href="/#contact">Let’s talk</a></header>
...
export function Work({ featured = false, bare = false }: { featured?: boolean; bare?: boolean }) {
  return <>
      <section id="work" className={styles.work}>{!bare && <div className={styles.sectionHeading} data-reveal><h2>{featured ? <>One process,<br />running without people.</> : <>Four processes<br />running without people.</>}</h2><p>{featured ? <>Patient bookings, from the phone call to the clinic record. <br /><a href="/work">See all four case studies</a></> : <>Each one used to need a person at every step. <br />The green line says where a human is still involved.</>}</p></div>}
        ...
        </TiltCard><div className={styles.projectCopy}><h3>{project.title}</h3><p>{project.description}</p><p className={styles.automated}>{project.handover}</p><p className={styles.numbers}>{project.numbers}</p><div className={styles.tags}>{project.tags.map(tag => <span key={tag}>{tag}</span>)}</div><details open={!featured}><summary>Inside the system <span aria-hidden="true">+</span></summary><div className={styles.caseStudy}><h4>How it works</h4><p>{project.detail}</p><h4>Engineering detail</h4><p>{project.decision}</p></div></details></div></article>)}
      </section>
  </>;
}
